...
def convert_array(array1, array2)
  new_array = []
  array1.each do |video|
    if video[:views] >= 100
      new_array << video
      array2.each do |author|
        if author[:id] === video[:author_id]
          video[:author_name] = author[:first_name] + " " + author[:last_name]
        end
      end
      video.delete(:author_id)
    end  
  end
  return new_array
end
...
def non_repeating_letter(string)
  letter = ""
  hash = {}
  string.each_char do |char|
    if hash[char]
...
# p non_repeating_letter("leetcode")
# p non_repeating_letter("loveleetcode")
# p non_repeating_letter("lleetcode")
# p non_repeating_letter("clovveleetode")
...
def hundred_views(array1, array2)
  new_array = []
  array1.each do |video|
    if video[:views] >= 100
      array2.each do |author|
